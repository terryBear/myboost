...
import { get, isAdminFromToken } from "@/services/api";
import {
    AlertCircle,
    AlertTriangle,
    ArrowLeft,
...
const SecurityDetails = () => {
  ...
  const fetchSecurityData = async () => {
    ...
    try {
      ...
      const [agentsRaw, devicesRaw] = await Promise.all([
      ...
      setSecuritySummary({
        ...
        antivirusInstalled: devices.length,
        sentinelOneInstalled: agents.length,
        protectedDevices: Math.max(devices.length, agents.length),
        unprotectedDevices: 0,
        activeThreats: 0,
      });

      const antivirusFormatted: AntivirusData[] = devices.map((d: any, i: number) => ({
        id: i,
        device_name: d.name || d.deviceName || d.hostname || "Unknown Device",
        av_product_name: "N/A",
        av_status: "Enabled",
        threat_count: 0,
        last_update: d.lastSeen || d.last_seen || new Date().toISOString(),
        last_scan: d.lastSeen || d.last_seen || new Date().toISOString(),
      }));
      const s1Formatted: SentinelOneData[] = agents.map((a: any, i: number) => ({
        id: i,
        endpoints: a.computerName || a.endpointName || a.hostname || "—",
        site: a.siteName || a.site || "—",
        status: a.agentStatus || "Protected",
        classification: a.classification || "Clean",
        confidence_level: a.confidenceLevel || "High",
        threat_details: "SentinelOne Agent",
        analyst_verdict: a.analystVerdict || "Clean",
        incident_status: a.incidentStatus || "Resolved",
      }));
