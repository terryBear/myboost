  ...
  const fetchPatchData = async (customerName: string) => {
    try {
      ...
      const [checksData, customersData] = await Promise.all([
        get<any[]>("reporting/checks/", { params }),
        get<any[]>("reporting/dashboard/customers/", { params }),
      ]);

      const checks = Array.isArray(checksData) ? checksData : [];
      const customers = Array.isArray(customersData) ? customersData : [];

      // Map failing checks to PatchData-like shape (device, client, site, patch, status, etc.)
      const filteredPatches: PatchData[] = checks.map((c: any, i: number) => ({
        id: i + 1,
        device: c.device ?? c.deviceName ?? c.deviceid ?? "",
        client: c.client ?? c.clientName ?? customerName ?? "",
        site: c.site ?? c.siteName ?? "",
        patch: c.description ?? c.patch ?? c.patchName ?? "Check",
        status:
          c.status === "failing" || c.status === "Failed"
            ...
            : c.status === "pending"
              ? "Pending"
              : "Installed",
        discovered_install_date: c.start_date ?? c.discovered_install_date ?? new Date().toISOString(),
        created_at: c.start_time ?? c.created_at ?? new Date().toISOString(),
      }));

      setPatchData(filteredPatches);

      // Compliance from dashboard customers (patchCompliance)
      const customerRow = customers.find(
        (c: any) =>
          String(c.name || "").toLowerCase() === customerName.toLowerCase() ||
          String(c.id || "").toLowerCase() === customerName.toLowerCase()
      );
      if (customerRow?.patchCompliance != null) {
        setCoffeeReportCompliance(Number(customerRow.patchCompliance));
        localStorage.setItem(
          "coffeeReportCompliance",
          String(customerRow.patchCompliance)
        );
      }
    } catch (error) {
